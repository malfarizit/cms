...
function public_template2($view, $data)
{
    $ci = get_instance();
    $ci->load->view('_includes/public/header2.php', $data);
    $ci->load->view($view);
    $ci->load->view('_includes/public/footer.php');
}
...
function get_department($id = null)
{
    ...
    if ($id == null) {
        return $ci->main->get('department');
    } else {
        $query = $ci->main->get_where('department', ['department_id' => $id]);
